...
from config import (
    AWS_DEFAULT_REGION, DATABASE_URL_PROD, DATABASE_URL_VALIDATION, RAG_LLM_DIR,
    RAG_LLM_VALIDATION_URL, RAG_S3_BUCKET, RAG_S3_PREFIX, WORK_DIR,
)

sys.path.insert(0, str(RAG_LLM_DIR))

# Répertoire fixe sous WORK_DIR (bind-mount work/, cf. dags/config.py) plutôt qu'un
# tempfile.mkdtemp() : vidé et réécrit à chaque run (cf. download_and_ingest_validation), donc pas
...
def branch_check_new_docs(**context) -> str:
    ...
    branche Neon de PROD. Retourne le prochain task_id ("download_and_ingest_validation" ou
    "stop_dag").
    """
    try:
    ...
    return "download_and_ingest_validation"


def download_and_ingest_validation(**context) -> str:
    """
    Télécharge les docs S3 dans RAG_KNOWLEDGE_WORK_DIR (work/rag-knowledge/, cf. module) et les
    ingère dans la branche Neon de validation. Le répertoire est vidé avant chaque téléchargement
    (pas seulement écrasé) pour éviter qu'un fichier supprimé côté S3 reste ingéré par erreur
    depuis un run précédent.
    """
    client = _s3_client()
    docs = _list_knowledge_docs()
    ...
    _run_ingestion(str(work_dir), DATABASE_URL_VALIDATION)

    context["ti"].xcom_push(key="knowledge_dir", value=str(work_dir))
    return str(work_dir)


def _wait_for_rag_llm_validation_ready(url: str, timeout: int = 180, interval: int = 5) -> None:
    """
    Poll GET {url}/health jusqu'à succès ou timeout. Le service Render vitiscan-rag-llm-validation
    est sur le plan free : après 15 min sans requête, il se met en veille et met jusqu'à ~30-90s à
    se réveiller (cf. docs/deploiement-render-streamlit.md) - sans cette attente, le premier appel
    ...
    while time.monotonic() < deadline:
        try:
            ...
            last_error = exc
            time.sleep(interval)
    raise RuntimeError(f"vitiscan-rag-llm-validation ({url}) ne répond pas sur /health après {timeout}s : {last_error}")
...
def run_golden_prompts_gate(**context) -> None:
    ...
    _wait_for_rag_llm_validation_ready(RAG_LLM_VALIDATION_URL)
    ...
    for case in cases:
        try:
            resp = requests.post(f"{RAG_LLM_VALIDATION_URL}/solutions", json=build_payload(case), timeout=90)
            resp.raise_for_status()
